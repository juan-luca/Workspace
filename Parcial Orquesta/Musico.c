...
void hardCodearMusicos(Musico listado[], int tam)
{
  ...
  int idOrquesta[5] = {101,103,105,101,102};
  int idInstrumento[5]={1001,1002,1003,1003,1003};
  int i;

  for(i=0; i<5; i++)
  {
      id[i]=generateIdMusico(listado,tam,lastIdMusico);
        lastIdMusico=id[i];
...
int removeMusicoByOrquestaID(Musico listaMusico[], int tam, int idOrquesta)
{
    int i, ret=-1;

   for(i=0; i<tam; i++)
   {
       if(listaMusico[i].idOrquesta==idOrquesta)
       {
           listaMusico[i].isEmpty=ELIMINADO;
           ret=0;
